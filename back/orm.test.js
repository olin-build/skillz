...
describe('constructQuery', () => {
    test("constructs INSERT statement and parameters", () => {
        let { insert, vars } = constructQuery({
        ...
        expect(vars).toEqual(['c', 'd', 'a', 'b'])
    });
    test("constructs UPDATE statement", () => {
        let { update, vars } = constructQuery({
            tableName: 'test_table',
            where: { col3: 'c', col4: 'd' },
            cols: ['col1', 'col2'],
            data: { col1: 'a', col2: 'b' }
    ...
    test("ignore data fields that aren't in the columns list", () => {
        let { insert, update, vars } = constructQuery({
            tableName: 'test_table',
            where: { col3: 'c', col4: 'd' },
            cols: ['col1', 'col2'],
