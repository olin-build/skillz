...
import { getPersonSkillRecords } from './EditPerson';

export const PersonSkillsTable = ({ data, editablePerson = null, onRowClick }) => {
  if (data.error) {
    console.error(data.error.message); /* eslint no-console: 0 */
    return (
  ...
  return (
    <table className="ui striped selectable definition table">
      <thead>
        <tr className="sticky">
          <th />
...
export const PersonSkillRow = ({
  person, skills, onClick, editable,
}) => {
